...
class TestFetchCli(TestElasticsearchIntegrationBase):
    ...
    def test_fetch_with_different_index(self):
        ...
        runner = CliRunner()
        first_result = runner.invoke(cli.cli, ['-v',
                                               '--connect', ES_HOST,
                                               '--username', USERNAME,
                                               '--password', PASSWORD,
                                               'fetch'])

        second_result = runner.invoke(cli.cli, ['-v',
                                                '--connect', ES_HOST,
                                                '--username', USERNAME,
                                                '--password', PASSWORD,
                                                'fetch',
        ...
        self.assertIn('Connecting to "localhost:9200" with "someUser"', first_result.output)
        ...
        self.assertIn('Connecting to "localhost:9200" with "someUser"', second_result.output)
...
#########
2019-01-01T10:02:00+00:00 secondLine
2019-01-01T10:03:00+00:00 thirdLine""", result.output)
        self.assertEqual(0, result.exit_code)
    ...
    def test_fetch_with_from_and_to_time(self):
        ...
        result = runner.invoke(cli.cli, ['-v',
                                         ...
                                         '--password', PASSWORD,
                                         'fetch',
                                         '--from-time', TIMESTAMP_ONE_HALF.strftime("%Y-%m-%dT%H:%M:%S%z"),
                                         '--to-time', TIMESTAMP_TWO_HALF.strftime("%Y-%m-%dT%H:%M:%S%z")])
