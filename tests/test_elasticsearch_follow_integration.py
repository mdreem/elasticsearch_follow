...
class TestElasticsearchFollowIntegration(TestElasticsearchIntegrationBase):
    def test_query_line(self):
        ...
        new_lines = list(
            es_follow.get_new_lines(
                "test_index",
                datetime(year=2019, month=1, day=1, hour=10, minute=0, tzinfo=tz.UTC),
            )
        )
    ...
    def test_query_lines_returned_ordered_by_timestamp(self):
        ...
        self.insert_line(
            ...
            timestamp=datetime(
                year=2019, month=1, day=1, hour=10, minute=2, tzinfo=tz.UTC
            ),
        )
    ...
    def test_query_string_returns_correct_results(self):
        ...
        es = Elasticsearch(["http://localhost:9200"])
        es_follow = elasticsearch_follow.ElasticsearchFollow(
            elasticsearch=es, query_string=query_string
        )
